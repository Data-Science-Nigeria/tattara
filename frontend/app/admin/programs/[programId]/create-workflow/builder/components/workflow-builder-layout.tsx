      ...
      <div>
        <button
          onClick={() => {
            ...
            const programId =
              programsIndex !== -1 ? pathParts[programsIndex + 1] : null;
            const searchParams = new URLSearchParams(window.location.search);
            const workflowId = searchParams.get('workflowId');

            if (programId) {
              if (workflowId) {
                window.location.href = `/admin/programs/${programId}/create-workflow/workflow-details?workflowId=${workflowId}`;
              } else {
                window.location.href = `/admin/programs/${programId}/create-workflow/workflow-details`;
              }
            } else {
              window.history.back();
            }
