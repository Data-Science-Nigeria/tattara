...
export default function ImageBuilder() {
  ...
  const programIdFromRoute = params.programId as string;
  ...
  const createWorkflowMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (workflow?.id) {
        ...
        router.push(
          `/admin/programs/${programIdFromRoute}/create-workflow/field-mapping?workflowId=${workflow.id}`
        );
      }
    },
    onError: (error) => {
  ...
  const handleSave = async () => {
    if (isEditMode && workflowId) {
      ...
      } else if (currentStep === 'image' && aiFieldMappings.length > 0) {
  ...
  const handleCreateWorkflow = async () => {
    ...
    try {
      await createWorkflowMutation.mutateAsync({
        body: {
          programId: programIdFromRoute || undefined,
          name: name!,
  ...
  const steps = [
    ...
    { id: 'image', label: 'Image & OCR Settings' },
          ...
          <div className="space-y-6">
            <div className="rounded-lg border border-green-200 bg-green-50 p-4">
              <h3 className="mb-2 text-sm font-medium text-green-900">
                Ready to Create Image Workflow
              </h3>
