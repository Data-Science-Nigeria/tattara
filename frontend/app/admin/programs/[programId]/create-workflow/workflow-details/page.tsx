...
export default function WorkflowDetails() {
  ...
  const handleNext = (name: string, description: string, type: string) => {
    if (isEditing && workflowId) {
      router.push(
        `/admin/programs/${programIdFromRoute}/create-workflow/builder/${type}?workflowId=${workflowId}`
      );
    } else {
      ...
      router.push(
        `/admin/programs/${programIdFromRoute}/create-workflow/builder/${type}?${params.toString()}`
      );
    }
  };

  const handleBack = () => {
    router.push(`/admin/programs/${programIdFromRoute}/create-workflow`);
  };
