...
interface AssignUsersWorkflowModalProps {
  isOpen: boolean;
  onClose: () => void;
  workflows: Array<{
    id: string;
...
export default function AssignUsersWorkflowModal({
  isOpen,
  onClose,
  workflows,
}: AssignUsersWorkflowModalProps) {
  ...
  const handleAssign = async () => {
    if (selectedUsers.length === 0 || !selectedWorkflow) return;

    try {
      await assignUsersMutation.mutateAsync({
        path: { workflowId: selectedWorkflow },
        body: { userIds: selectedUsers },
      });

      const selectedWorkflowName = activeWorkflows.find(
        (w) => w.id === selectedWorkflow
      )?.name;
      toast.success(
        `Successfully assigned ${selectedUsers.length} user(s) to ${selectedWorkflowName}`
      );
      ...
      toast.error('Failed to assign users to workflow');
        ...
        <div className="mb-4 flex items-start justify-between sm:mb-6">
          <div>
            ...
            <p className="text-xs text-[#7987A0] sm:text-sm">
              Select users and workflow to create assignments
            </p>
          </div>
          <button
            onClick={onClose}
